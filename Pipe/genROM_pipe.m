...
alpha_max    = 1;                   % Máximum multiple of fundamental wavenumber (stremwise direction)
...
Nx          = 4*alpha_max + 2;      % Streamwise discretisation
nmodesy     = 16;                  % Controllability modes per wavenumber

m           = (-m_max:m_max)*(2*pi/Lm);  % Azimuthal wavenumber vector
alphas      = (0:alpha_max)*(2*pi/Lx);    % Streamwise wavenumber vector
...
out = "ROM_Galerkin_pipe_" + length(alphas) + "_alphas_" + length(m) + "_ms_" + "_alpha_max_" + alpha_max + "_m_max_" + m_max + ...
    "_Ncontrol_" + nmodesy + "_Nr_" + Nr + "_Nx_" + Nx + "_Nm_" + Nm + "_Modes_" + nmodes + "_Re_" + Re_bas +".mat";
