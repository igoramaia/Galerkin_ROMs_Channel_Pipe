%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Generate ROM for Couette flow using balanced modes
% The controllability and observability Gramians are approximated using resolvent analysis
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
phiauxH     = zeros(size(X,1),size(X,2),size(X,3),3,size(a,1),size(a,2),nmodesy);
...
for i=1:size(a,1)
    for j=1:size(a,2)
        [q,qH]                     = modalbasis_bal_res(a(i,j),g(i,j),Ulam,Re_bas,nmodesy,x,z);
        avec(i,j,:,:)              = a(i,j);
        gvec(i,j,:,:)              = g(i,j);
        for k=1:nmodesy
            bvec(i,j,k,:) = k;
            ...
            phi2                   = reshape(qH(:,k),[3*np,Nx,Nz,1]);
            
            phiaux(:,:,:,1,i,j,k)  = phi1(1:np,:,:);
            phiaux(:,:,:,2,i,j,k)  = phi1(np+1:2*np,:,:);
            phiaux(:,:,:,3,i,j,k)  = phi1(2*np+1:3*np,:,:);
            
            phiauxH(:,:,:,1,i,j,k) = phi2(1:np,:,:);
            phiauxH(:,:,:,2,i,j,k) = phi2(np+1:2*np,:,:);
            phiauxH(:,:,:,3,i,j,k) = phi2(2*np+1:3*np,:,:);
        end
    end
end
...
for i=1:nmodes
    ...
    for j=1:nmodes
        M(i,j) = inprod_s(squeeze(phiH(:,:,:,:,i)),squeeze(phi(:,:,:,:,j)),Wf);
    end
end
...
for i=1:nmodes
    ...
    for j=1:nmodes
        L(i,j)          = inprod_s(phiH(:,:,:,:,i),Lapl(:,:,:,:,j),Wf);
    end
end
...
outfile = ['galerkin_model_bal_' int2str(nalphas) '_' int2str(nmodesy) '_' int2str(ngammas) '_Lx_' num2str(Lx/pi) '_Lz_' num2str(Lz/pi) '_Re_' num2str(Recalc) '.mat'];
save(outfile,'L','L2','L3','L4','Q','QQ','phi','phiH','phi0','phix','phiy','phiz','X','Y','Z','Wf','avec','bvec','gvec','kvec','alphas','gammas','Nx','Ny','Nz','-v7.3');
